...
\subsection{Predictions}
    ...
    \subsection{Analysis of Model}
	 This model suffer from being dependent upon too many parameters, and it is hard to come up with an optimal vaccine distribution. Tweaking the parameters a little bit can have a drastic effect on the model, and it relies too much on our research being accurate. In the future I would search for a pareto optimal distribution. Also, I would fix the assumption of a linear vaccination rate so we are not wasting vaccines on a completely vaccinated population, and can test getting all infants out of the way at the beginning.
